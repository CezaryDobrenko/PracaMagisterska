...
from scrapper.tests.factories import FolderFactory, RequestFactory, UserFactory, ApiKeyFactory
...
class FolderTest(TestCase):
    ...
    def test_authenticated_user_update_folder_mutation(self):
        folder = FolderFactory(name="old_name", is_ready=False, scraping_interval="MINUTE5", user=self.user)
        is_ready = "true"
        ...
        expected = {'updateFolder': {'folder': {'name': new_name, 'isReady': True, 'scrapingInterval': scraping_interval}}}
    ...
    def test_authenticated_user_update_other_user_folder_mutation(self):
        ...
        assert result["errors"][0]["message"] == "This folder cannot be edited by given api_key"
