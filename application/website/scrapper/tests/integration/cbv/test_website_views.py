...
class WebsiteCBVTests(TestCase):
    ...
    def test_creator_list_view(self):
        ...
        response = self.logged_client.get(reverse(self.creator_list_view, args=(self.folder.id,)))
        expected = list(Website.objects.all())
    ...
    def test_create_view(self):
        ...
        response = self.logged_client.post(
            reverse(self.create_view, args=(self.folder.id,)), data=form_data, follow=True
        )
    ...
    def test_update_view(self):
        website = WebsiteFactory(url="www.website_1.com", description="old_desc", is_ready=False, is_simplified=False, folder=self.folder)

        response = self.logged_client.post(
            reverse(self.update_view, args=(website.id,)),
            data={"url": "https://www.wp.pl/", "description": "opis", "is_ready": "on", "is_simplified": "on"},
            follow=True,
        )
